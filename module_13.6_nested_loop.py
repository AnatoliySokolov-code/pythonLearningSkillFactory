...
# Для того, чтобы обойти всю таблицу, нужен цикл, который будет
# последовательно перебирать все строки, а также цикл, который будет
# перебирать все столбцы для каждой строки. То есть обход матрицы будет
# слева-направо, сверху-вниз.
#
# Обычно индекс, который отвечает за строки обозначают как i, а индекс,
# отвечающий за столбцы — j, тогда:
...
# 0 1 2 3 4 5
# Видим, что наша двумерная матрица распечатана в виде одномерной.
# Хотя все элементы последовательно расположены друг
# за другом. Чтобы она была распечатана в виде таблицы, нужно добавить print(),
# который будет выполняться после обработки каждой строки:роки:
...
# 0 1 2
# 3 4 5
# Разберем большой пример работы с таблицами.
...
# Так как таблица представляет собой список списков,
# первый цикл будет принимать целую строку.
# Мы не знаем значение минимального элемента, поэтому возьмем первый элемент
# в качестве минимального и будем искать элемент меньше него.
# Аналогично поступим с максимальным элементом.
